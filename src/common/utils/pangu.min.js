              ...
              var c = (function(e) {
                  function t() {
                    ...
                    return (
                      ...
                      (e.presentationalTags = /^(b|code|del|em|i|s|strong|kbd)$/i),
                      ...
                      },
                      {
                        ...
                        value: function(e, t) {
                          if (t instanceof Node && !(t instanceof DocumentFragment))
                            for (var n, a, i = document.evaluate(e, t, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null), o = i.snapshotLength - 1; o > -1; --o) {
                              if (
                                ((n = i.snapshotItem(o)), this.isSpecificTag(n.parentNode, this.presentationalTags) && !this.isInsideSpecificTag(n.parentNode, this.ignoredTags))
                              ) {
                                var r = n.parentNode;
                                    ...
                                    if (-1 === v.nodeName.search(this.blockTags))
                                      if (-1 === y.nodeName.search(this.spaceSensitiveTags))
                                        -1 === y.nodeName.search(this.ignoredTags) &&
                                          ...
                                          (a.previousSibling
                                            ? -1 === a.previousSibling.nodeName.search(this.spaceLikeTags) && (a.data = ' '.concat(a.data))
                                            : this.canIgnoreNode(a) || (a.data = ' '.concat(a.data)));
                                      else if (-1 === v.nodeName.search(this.spaceSensitiveTags)) n.data = ''.concat(n.data, ' ');
                                      else {
                                        var b = document.createElement('pangu');
                                        (b.innerHTML = ' '),
                                          y.previousSibling
                                            ? -1 === y.previousSibling.nodeName.search(this.spaceLikeTags) && y.parentNode.insertBefore(b, y)
                    ...
                    (j = [
                      ...
                      },
                      {
                        ...
                        value: function(e) {
                          ...
                          })).replace(c, '$1 $2')).replace(s, '$1：$2')).replace(u, '$1 $2')).replace(p, '$1 $2')).replace(l, '$1$2$3')).replace(f, '$1 $2')).replace(
                            g,
                            '$1 $2'
                          )).replace(d, "$1's")).replace(h, '$1 $2$3$4 $5')).replace(y, '$1 $2')).replace(v, '$1 $3')).replace(b, '$1 $2 $3')).replace(m, '$1 $2 $3')).replace(
                            $,
                            '$1$2'
                          )).replace(E, '$1$2$3')).replace(S, '$1 $2')).replace(T, '$1 $2')).replace(N, '$1$2$3')).replace(w, '$1 $2$3$4')).replace(k, '$1$2$3 $4')).replace(
                            P,
                            '$1 $2'
                          )).replace(O, '$1 $2')).replace(_, '$1 $2')).replace(x, '$1 $2')).replace(R, '$1 $2')).replace(A, '・'));
                        },
                      },
                      {
                        key: 'spacingText',
